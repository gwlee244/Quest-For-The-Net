...
$.get("/api/all", (data) => {
    if (data.length !== 0) {
        ...
        <div class="row player-div">
            <div class="col-md-6">
            <button type="submit" class="addbtn  center-block off-set-70" name="addTo" data-id="${data[i].id}">Add to Roster</button>
            <button type="submit" name="remove" class="removebtn  center-block off-set-70">Remove from Team</button>
            <div class="player-card">
                    <div class="player-card__side player-card__side--front">
                        <div class="player-card__picture player-card__picture--1">
                            &nbsp;
...
$.ajax({
    ...
    .then(function(response) {
      $(".addedPlayers").prepend(addplayer);
        console.log("response", response);
       //window.location.reload();
    });
})
